...
class graph:
    ...
    def dfs(self, rand, adj, colors, parents,out):
        ...
        colors[i]="b"
        return(out)
    def bfs(self,rand,visited,level,bfs_output,q,parent):
        self.rand=rand
        ...
        while not self.q.empty():
            ...
            for v in self.adj_list[u]:
                if not self.visited[v]:
                    ...
                    self.level[v]=self.level[u]+1
                    self.q.put(v)
        # return(self.level[self.find_distance])   ## to know the distance of a specific vertex
    ...
    def kruskalsAlgo(gra,vertices):
        ...
        sorted_values = sorted(p.items(), key=lambda x:x[1]) # Sort the values
    ...
    def find_cycle_dfs(self,rand,adj,colour,parent,out):
        ...
        for v in self.adj[self.rand]:
            if self.colour[v]=="w": #and self.parent[v]!=self.rand: -> Do this for undirected graph
                c=self.find_cycle_dfs(v,self.adj,self.colour,self.parent,self.out)
                if c==True:
                    return(True)
...
# bfs_output=[]
# q=Queue()
# o=gr.bfs("f",visited,level,bfs_output,q,parent)
# print(o)                       ## end of BFS input



# nodes=["a","b","c","d","e","f"]  ## find the distance of all vertex input start
# gr=graph(nodes)
# edges=[("a","b"),("a","c"),("b","d"),("b","e"),("e","f"),("c","f")]
# # edges=[("a","b"),("a","e"),("b","c"),("b","e"),("c","d"),("d","e")]
# for u,v in edges:  
...
# q=Queue()
# find_distance="a"
# o=gr.shortest_distance_from_node("f",visited,level,bfs_output,q,parent,find_distance)  ##here "f" is considered to be source node
...
# # edges=[("a","b"),("a","c"),("b","d"),("b","e"),("e","f"),("c","f")]
# # edges=[("a","b"),("a","e"),("b","c"),("b","e"),("c","d"),("d","e")]
# edges=[('a','b'),('a','d'),('b','c'),('d','e'),('d','f'),('e','f'),('e','g'),('f','h'),('g','h')]
# for u,v in edges:
#     gr.add_edge(u,v)
# k=gr.print_graph()
# # print(k)
...
# path=[]
# find_path="g"
# o=gr.shortest_path("a",visited,level,bfs_output,q,parent,path,find_path)
# print(o)                       ## end of shortest path input




# # nodes=["a","b","c","d","e"]    ##find cycle in directed and undirected graph
...
# else:
#     print("the graph is not cyclic")   ##end of finding cycle in directed and undirected graph
